...
struct SpriteInfo {
public:
    SpriteInfo(std::string filename_, int totalWidth_, int totalHeight_, int spriteWidth_, int spriteHeight_,
               int cols_ = 1, int frames_ = 1)
        : filename((std::filesystem::path(ASSET_DIR) / filename_).string()), totalWidth(totalWidth_),
          totalHeight(totalHeight_), spriteWidth(spriteWidth_), spriteHeight(spriteHeight_), cols(cols_),
          frames(frames_) {
    }
...
private:
    ...
    constexpr SpriteInfoID EncodeAnim(ImageID imgID, AnimID animID) const {
        return imgID * 1000 + animID;
    }
